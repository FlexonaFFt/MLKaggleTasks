...
RUN_LABEL = "front02_legacy_front_k2_tail"
...
def _k2(index: int) -> AttackCandidate:
    return _candidate(
        K2_TEMPLATE.format(
            url1=_url(TAIL_START_IDX + 2 * index),
            url2=_url(TAIL_START_IDX + 2 * index + 1),
            payload=PAYLOAD,
        )
    )
