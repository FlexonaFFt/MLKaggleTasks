...
# --- AD telemetry (stderr only; no effect on scoring) ---
def _ad_log(msg: str) -> None:
    print("[AD] " + msg, file=sys.stderr, flush=True)


def _ad_fmt(name: str, s: dict) -> str:
    lats = sorted(s["lats"])
    if not lats:
        return name + ": n=0"
    def _q(p):
        return lats[min(len(lats) - 1, int(p * len(lats)))]
    return (f"{name}: n={s['n']} fired={s['fired']} "
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(
        ...
        try:
            ...
            _ad_log("warmup failed; env not probeable")
            return []
        tail_n_cfg = self.config.get("tail_n", TAIL_N)
        _ad_log(
            f"cfg budget={budget:.0f}s frac={frac} safe_frac={replay_safe_frac} "
            f"cap={cap} tail_n={tail_n_cfg} fire={fire_check_mode} "
            f"split_n={split_classify_n} thr={split_threshold:.1f} probe_hops={probe_hops} "
            f"coef={replay_cost_coef} burst_k={burst_k}"
        )
        _ad_log(f"warmup ok elapsed={time.monotonic() - run_start:.1f}s")
        ...
        while len(cands) < cap:
            ...
            if burst_k > 1:
                ...
                tpl_name = "frame" if chosen_template == frame_template else "plain"
            t0 = time.monotonic()
            try:
                env.reset()
                ...
                _ad_log(f"env error at trial {trial_count}; returning fired-so-far")
            ...
            if classifying:
                ...
                if classify_n == split_classify_n:
                    ...
                    branch = "FRAME" if chosen_template == frame_template else "PLAIN"
                    _ad_log(
                        f"classify done n={classify_n} "
                        f"mean={classify_sum / classify_n:.2f}s "
                        f"thr={split_threshold:.1f}s -> {branch}"
                    )
            trial_count += 1
            _s = tpl_stats.setdefault(tpl_name, {"n": 0, "fired": 0, "lats": []})
            _s["n"] += 1
            _s["lats"].append(elapsed)
            if fired:
                _s["fired"] += 1
            if tpl_name == "frame" and not rolled_back:
                frame_n += 1
                frame_fired += 1 if fired else 0
                if frame_n == 20 and frame_fired < 12:
                    rolled_back = True
                    chosen_template = TEMPLATE
                    _ad_log(f"ROLLBACK frame->plain after {frame_fired}/20 live fires")
            if trial_count % 200 == 0:
                _ad_log(" | ".join(_ad_fmt(k, v) for k, v in sorted(tpl_stats.items())))
            if fired:
        ...
        if cands:
            ...
            while len(cands) < tail_target:
                ...
                idx += 1
            ttpl = "FRAME" if tail_template == frame_template else "PLAIN"
            _ad_log(f"tail filled to {len(cands)} tpl={ttpl}")
        for _k in sorted(tpl_stats):
            _ad_log("final " + _ad_fmt(_k, tpl_stats[_k]))
        kept_total = sum(v["fired"] for v in tpl_stats.values())
        wall = time.monotonic() - run_start
        _ad_log(
            f"final returned={len(cands)} kept={kept_total} "
            f"replay_cost={replay_cost:.0f}/{replay_cap:.0f} "
            f"wall={wall:.0f}s slowest={slowest:.2f}"
        )
        return cands
