...
#
# Variant: fill_unit estimated via 90th percentile of the selected template's
# probe latencies instead of the median. The median can understate the true
# per-candidate replay cost of a template with a heavy right tail (occasional
# slow/CoT-heavy trials), which under-fills replay_cost during the fill loop
# and then relies on the end-of-run hard clamp to fix it after the fact --
# trimming already-earned candidates rather than pacing fill correctly from
# the start. A 90th-percentile estimate is a more conservative per-candidate
# cost, so fewer fill candidates get attempted, but the ones that are kept
# should need the hard clamp less often (fewer earned candidates thrown away
# at the end). Net effect on score is not obvious a priori -- that is exactly
# what this submission is measuring, purely via the resulting score delta vs
# the 87.705 baseline (no log access assumed).
# =====================================================================================
...
def _percentile(values: list[float], pct: float) -> float:
    """Nearest-rank percentile (no interpolation needed at our small sample
    sizes). pct in [0, 100]."""
    ordered = sorted(values)
    if not ordered:
        return float("inf")
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # A per-candidate replay estimate for the fill template: the 90th
        # percentile of probe latencies (falls back to the slowest observed if
        # unmeasured). More conservative than the median -- accounts for a
        # heavy right tail without needing the full max, which a single
        # outlier trial could distort.
        selected_latencies = latencies[selected_index]
        fill_unit = _percentile(selected_latencies, FILL_UNIT_PERCENTILE) if selected_latencies else slowest
