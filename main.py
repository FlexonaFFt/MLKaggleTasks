...
_PREMIUM_ITEMS = ("STRAWBERRY", "MELON", "MILK", "WOOL")
_PREMIUM_MAX_SHIFT = 4
_PREMIUM_STATE = {0: {}, 1: {}}


def _premium_distance(obs):
    farms = list(_get(obs, "farms", []) or [])
    if len(farms) != 2:
        return 99
    left, right = farms
    ...
    if tuple(_get(left, "farmer", []) or []) != tuple(_get(right, "farmer", []) or []):
        distance += 2
    lh = [tuple(x or ()) for x in (_get(left, "hands", []) or [])]
    rh = [tuple(x or ()) for x in (_get(right, "hands", []) or [])]
    distance += 2 * abs(len(lh) - len(rh)) + sum(a != b for a, b in zip(lh, rh))
    distance += 2 * len(set(_get(left, "unlocked_quadrants", []) or []).symmetric_difference(
        set(_get(right, "unlocked_quadrants", []) or [])
    ))
    return distance


def _premium_order(order):
    if len(order) >= 3 and order[0] == "SELL" and order[1] in _PREMIUM_ITEMS:
        return order[1], max(0, int(order[2] or 0))
    return None, 0


def _premium_shift_available(obs, item, current_quantity):
    private = _get(obs, "private", {}) or {}
    shed = _get(private, "shed", {}) or {}
    available = int(_get(shed, item, 0) or 0)
    return max(0, min(_PREMIUM_MAX_SHIFT, available - current_quantity))


def _premium_repay(action, obs, step):
    state = _PREMIUM_STATE[_seat(obs)]
    if state.get("due_step") != step:
        return action
    item, quantity = state.get("item"), int(state.get("quantity", 0) or 0)
    for index, order in enumerate(list(action.get("market") or [])):
        name, current = _premium_order(order)
        if name != item:
            continue
        market = list(action.get("market") or [])
        updated = list(order)
        if current > quantity:
            updated[2] = current - quantity
...
def _premium_preempt(action, obs, step):
    if _premium_distance(obs) > 2 or step + 1 >= len(_ACTIONS):
        return action
    future = next((_premium_order(order) for order in ((_ACTIONS[step + 1] or {}).get("market") or [])
                   if _premium_order(order)[0] in _PREMIUM_ITEMS), (None, 0))
    future_item, future_quantity = future
    if not future_item or future_quantity <= 0:
        return action
    market = list(action.get("market") or [])
    for index, order in enumerate(market):
        item, quantity = _premium_order(order)
        if item != future_item or quantity <= 0:
            continue
        shift = _premium_shift_available(obs, item, quantity)
        if shift <= 0:
            return action
        updated = list(order)
        updated[2] = quantity + shift
        market[index] = updated
        _PREMIUM_STATE[_seat(obs)] = {"due_step": step + 1, "item": item, "quantity": shift}
        action["market"] = market
        return action
    return action
...
def agent(obs):
    try:
        step = min(max(0, int(_get(obs, "step", 0) or 0)), len(_ACTIONS) - 1)
        action = _weed_repair_action(obs, _copy_action(_ACTIONS[step]), _ACTIONS, step)
        action = _premium_repay(action, obs, step)
        action = _impact_slots(obs, action)
        action = _premium_preempt(action, obs, step)
        return _align_hands(_impact_slots(obs, action), obs)
    except Exception:
        farm = _farm(obs, _seat(obs))
