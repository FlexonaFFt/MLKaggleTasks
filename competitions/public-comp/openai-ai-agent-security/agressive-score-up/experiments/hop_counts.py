...
#
# Variant: DIAGNOSTIC-ONLY instrumentation (item #4 from strategy review).
# Question being tested: does the gateway's forced max_tool_hops=8 replay cost
# scale with the *requested* hop ceiling, or with the *actual* number of hops
# a trial consumes before the agent stops on its own? Templates like inj_close
# are designed to collapse the model to a 1-hop finish; if replay cost tracks
# actual consumed hops (not the ceiling), a template that reliably finishes in
# 1 hop is structurally cheaper at replay than one that meanders even if both
# are probed at the same max_tool_hops=8 ceiling. This variant adds hop-count
# extraction from the trace and logs per-template hop/latency stats. It changes
# NO selection or sizing logic — same candidates would be returned as the
# parent notebook. This is measurement-only, safe to submit as-is.
# =====================================================================================
...
def _hop_count(trace: Mapping[str, Any]) -> int:
    """Best-effort count of actual tool-call hops consumed in a trial, as
    opposed to the requested max_tool_hops ceiling. Diagnostic only: tells us
    whether templates that are designed to collapse reasoning also collapse
    the *number* of hops, or only the token count within a hop. Tries the
    most specific field first and falls back gracefully across SDK versions."""
    events = trace.get("tool_events")
    if isinstance(events, list):
        return len(events)
    hops = trace.get("hops") or trace.get("num_hops") or trace.get("hop_count")
    if isinstance(hops, int):
        return hops
    turns = trace.get("turns") or trace.get("messages")
    if isinstance(turns, list):
        return sum(1 for t in turns if isinstance(t, Mapping) and t.get("role") == "tool")
    return -1  # unknown / could not extract
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # Diagnostic only: actual hops consumed per trial (vs the max_tool_hops=8
        # ceiling requested). -1 entries mean the trace didn't expose a countable
        # field for this SDK build; those are filtered out of the stats below.
        hop_counts: list[list[int]] = [[] for _ in TEMPLATES]
        ...
        def trial(template_index: int, index: int) -> tuple[bool, float]:
            ...
            if trace is not None:
                hops = _hop_count(trace)
                if hops >= 0:
                    hop_counts[template_index].append(hops)
            if fired:
        ...
        if search_time_left():
            ...
            hop_counts[FALLBACK_INDEX].clear()
            bank.clear()
            bank_seen.clear()
        ...
        # Diagnostic (item #4): actual hops consumed per template vs the
        # max_tool_hops=8 ceiling that was requested for every trial. If these
        # numbers cluster well below 8, replay cost is very unlikely to be paying
        # for a fixed 8-hop ceiling — it is tracking real, per-trial conversation
        # length, which means single-hop-collapsing templates (inj_close /
        # inj_commentary) are structurally cheaper at replay than the raw
        # max_tool_hops setting alone would suggest. "n/a" means this SDK build's
        # trace didn't expose a countable hop field — see _hop_count().
        def _hop_summary(counts: list[int]) -> str:
            if not counts:
                return "n/a"
            return "min%d/med%.1f/max%d(n=%d)" % (
                min(counts), _median([float(c) for c in counts]), max(counts), len(counts),
            )

        hop_summary = ",".join(
            "%s:%s" % (TEMPLATES[index][0], _hop_summary(hop_counts[index]))
            for index in range(len(TEMPLATES))
        )
        print(
        ...
        print(
            "[v26_hopdiag] requested_max_tool_hops=%d actual_hops_by_template: %s"
            % (max_tool_hops, hop_summary),
            file=sys.stderr,
            flush=True,
        )
